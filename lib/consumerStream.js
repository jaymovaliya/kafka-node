...
ConsumerStream.prototype.fetch = function () {
  ...
  if (self.ready && !self.fetchInFlight) {
    ...
    var encoder = self.showTimestamp ? protocol.encodeFetchRequestV2(self.fetchMaxWaitMs, self.fetchMinBytes) : protocol.encodeFetchRequest(self.fetchMaxWaitMs, self.fetchMinBytes);
    var decoder = self.showTimestamp ? protocol.decodeFetchResponseV1(self.decodeCallback.bind(self), self.maxTickMessages) : protocol.decodeFetchResponse(self.decodeCallback.bind(self), self.maxTickMessages);
