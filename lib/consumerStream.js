...
ConsumerStream.prototype.fetch = function () {
  ...
  if (self.ready && !self.fetchInFlight) {
    ...
    var encoder = options.showTimestamp ? protocol.encodeFetchRequestV2(options.fetchMaxWaitMs, options.fetchMinBytes) : protocol.encodeFetchRequest(options.fetchMaxWaitMs, options.fetchMinBytes);
    var decoder = options.showTimestamp ? protocol.decodeFetchResponseV1(self.decodeCallback.bind(self), options.maxTickMessages) : protocol.decodeFetchResponse(self.decodeCallback.bind(self), options.maxTickMessages);
