...
Client.prototype.sendFetchRequest = function (consumer, payloads, fetchMaxWaitMs, fetchMinBytes, maxTickMessages, showTimestamp) {
  var encoder = showTimestamp ? protocol.encodeFetchRequestV2(fetchMaxWaitMs, fetchMinBytes) : protocol.encodeFetchRequest(fetchMaxWaitMs, fetchMinBytes);
  // TODO: state validator for HLC for ignoring stale fetch requests
  var decoder = showTimestamp ? protocol.decodeFetchResponseV1(this._createMessageHandler(consumer), maxTickMessages) : protocol.decodeFetchResponse(this._createMessageHandler(consumer), maxTickMessages);
